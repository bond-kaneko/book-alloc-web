...
interface IPie {
  ...
  export class PieChart {
    ...
    private readonly colors = ['#34a853', '#FBBC05', '#EA4335'];
    ...
    private getData(elem: HTMLCanvasElement): IParts[] {
      ...
      if (Array.isArray(values) && Array.isArray(labels)) {
        ...
        values.forEach((value, idx) => {
          const endAngle = startAngle + value / sum * 2 * Math.PI;
        ...
        throw new TypeError('Data missing or not set properly');
      }
    }
    ...
    private onMouseOver(event: MouseEvent) {
      ...
      if (radius <= this.pie.radius) {
        ...
        const tipWidth = this.tipContext.measureText(this.getInfo(dataIdx)).width;
